...
@Component({
  selector: 'app-game',
  standalone: true,
  imports: [RouterModule, MatIconModule, CommonModule, FormsModule, MatSelect, MatOption, MatButtonModule, MatFormFieldModule, TopoComponent],
...
export class GameComponent implements OnInit, OnDestroy {
  ...
  moles: number[] = new Array(BOARD_CELLS).fill(0).map((x, i) => i);
  visibleMoles: number[] = [];
  /**
  ...
  constructor(private route: ActivatedRoute, private _snackBar: MatSnackBar, private gameService: GameService) {
  }
  ...
  private openSnackBar() {
    this._snackBar.open(`Good Job ${this.username}!`, '', {duration: NOTIFICATION_DURATION});
  }
}
